...
# Data description
The data used in this document is collected in 2024, and describes the vegetation and occurrence of insects in four habitat types: - Sälg (1st)\ - Älvkant (1st)\ - Ö (4st)\ - Gräsmatta (1st bruksgräsmatta and 1st paradgräsmatta). Data is analysed on the level of vegetation, insects and pollinators.
...
plant_data_2024 <- plant_data_2024 %>% # Exclude plant species that are grasses or trees
...
```{r build plant df, include = FALSE}
...
```{r calculate total number of plant species in 2024, include = FALSE}
plant_SR_2024 = length(unique(plant_data_2024$Species_sv)) # Calculate 

print(plant_SR_2024)
```

The total number of plant species in the dataset is `print(plant_SR_2024)`. They are distributed over the different sites as follows:

```{r plotting plant data, per week, include = TRUE}
...
# Function to add rank based on abundance within each habitat type
add_species_rank <- function(data) {
  # Group by Site_type and rank species within each group, allowing ties
  data$Rank <- ave(data$Abundance, data$Site_type, FUN = function(x) rank(-x, ties.method = "min"))
  return(data)
}
# Apply the function to your dataset
df_insectspecies <- add_species_rank(df_insectspecies)

str(df_insectspecies)

# Plot species-rank curve
speciesrankplot_2024 <- ggplot(df_insectspecies, aes(x = Rank, y = Abundance, color = Site_type, group = Site_type)) +
  geom_line(linewidth = 1) +  # Draw lines for each habitat
  geom_point(size = 2) + # Optional: Add points for each species
  scale_x_continuous(breaks = seq(0, 15, by = 2)) +
  scale_y_continuous() +      # Log-scale for abundance (optional, enhances visibility for large differences)
  labs(title = "Species-Rank Curves by Habitat Type", x = "Species Rank", y = "Abundance", color = "Habitat Type") +
  theme_minimal() +       # Clean theme
  theme(legend.position = "right",
        plot.title = element_text(hjust = 0.5, size = 16),
        axis.title = element_text(size = 12))
speciesrankplot_2024

...
plant_data_2024 <- read_excel("./Data/LoL_data_JH20241031.xlsx", sheet = "LoL_veginfo")
...
```{r specaccum}
insect_data_2024 <- insect_data_2024 %>%  
  filter(!is.na(Species_lt)) %>% # Remove NAs for species names in Latin
  as.data.frame()

df_insectspecies <- ddply(insect_data_2024, ~ Site_type*Species_lt, summarise, # Calculate Abundance of each species for each habitat type
                         Abundance = as.numeric(length(Species_lt)))
 
df_insectranks <- df_insectspecies %>%
    group_by(Site_type) %>%  # Group by habitat type
    #arrange(Site_type, desc(Abundance)) %>%  # Sort species by abundance within each habitat
    mutate(Site_type, Abundance_rank = dense_rank(desc(Abundance))) %>%  # Rank species within each habitat
    ungroup()  # Remove grouping for a clean output

df_insectranks <- df_insectspecies %>%
    group_by(Site_type) %>%  # Group by habitat type
    arrange(Site_type, desc(Abundance)) %>%  # Arrange by Site_type and abundance within each group
    mutate(Abundance_rank = dense_rank(desc(Abundance))) %>%  # Rank species within each habitat based on abundance
    ungroup()

# Function to add rank based on abundance
calculate_abundance_rank <- function(df_insectspecies) {
  df_insectspecies %>%
    arrange(desc(Abundance)) %>%
    mutate(Abundance_rank = dense_rank(desc(Abundance)))
}

# Function to merge abundance and rank back into the original dataset
add_abundance_and_rank <- function(df, abundance_df) {
  left_join(df, abundance_df, by = "species")
}

# Main function to process the insect data
process_insect_data <- function(filepath, sheet = 1) {
  # Step 1: Load data
  df <- load_data(filepath, sheet)
  
  # Step 2: Calculate abundance per species
  abundance_df <- calculate_abundance(df)
  
  # Step 3: Calculate abundance rank
  abundance_df <- calculate_abundance_rank(abundance_df)
  
  # Step 4: Merge back with the original data
  final_df <- add_abundance_and_rank(df, abundance_df)
  
  return(final_df)
}

# Example usage
filepath <- "path/to/your/insect_observations.xlsx"  # Update this with your actual file path
final_dataset <- process_insect_data(filepath)
print(final_dataset)

# Function to generate the species-rank curve
species_rank_curve <- function(df) {
  # Ensure data is ordered by rank
  df <- df %>%
    arrange(abundance_rank)
  
  # Plot species-rank curve
  ggplot(df, aes(x = abundance_rank, y = abundance)) +
...
# Assuming `final_dataset` from the previous step contains abundance and rank
species_rank_curve(final_dataset)
```
...
plot_pollinatorAb_2024 <- ggplot(df_pollinators_2024, aes(x = as.factor(Week), y = Abundance, fill = Site)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_y_continuous(name = "Antal pollinatörer", limits = c(0, 60)) +
  scale_fill_manual(values = site_colors) +
  labs(x = "Vecka", fill = "Lokal") +
  theme_classic() +
    theme(strip.text = element_text(size = 14, face = "bold")) +
...
```{r build aggregated pollinator df}
...
```{r plotting insect SR over plant SR}
str(df_plants_2024)
str(df_insects_2024)

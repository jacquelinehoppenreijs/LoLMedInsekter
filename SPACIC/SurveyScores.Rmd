...
SPACIC_plot <- ggplot(SPACIC_data, aes(x = Moment, y = Mean, group = Group)) +
  geom_line(aes(color = Group),size = 1.2) + # Line connecting the points
  geom_point(aes(color = Group),size = 4) + # Points for means
  geom_errorbar(aes(ymin = Mean - SE, ymax = Mean + SE, color = Group), width = 0.2, size = 1.2) + # Error bars
  scale_color_manual(values = c("Control" = "gray", "Intervention" = "black")) +
  scale_x_discrete(name = "") +
  scale_y_continuous(name = "Mean SPACIC ± SE", limits = c(1, 5)) +
  theme_classic(base_size = 20) + # Clean theme with larger font
  theme(legend.position = c(0.05, 0.95), # Position in top-left corner (normalized coordinates)
    legend.justification = c("left", "top"), # Align legend to the top-left corner
    legend.title = element_blank(), # Remove legend title
    legend.background = element_rect(fill = "white", color = "black", size = 0.5)) # Legend position
SPACIC_plot
